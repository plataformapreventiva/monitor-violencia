...
$\mu_{t_0}$ y $\phi$.
- Para declarar una anomalía, se requieren también al menos 5 casos en los últimos 
3 meses (incluyendo el mes que se está evaluando).
...
```{r}
tiene_alarma <- function(monitor){
    any(monitor$monitor$data$alarm == 1)
}
monitores_alarma <- keep(monitores_mex, tiene_alarma)
```
